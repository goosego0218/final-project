...
public class OracleSocialMetricsRepository : ISocialMetricsRepository
{
    ...
    )
    {
        ...
        try
        {
            ...
            {
                _logger.LogInformation(
                    "[OracleSocialMetricsRepository] TikTok 메트릭 저장 성공 - post_id={PostId}, view={View}, like={Like}, comment={Comment}, share={Share}",
                    postId,
                    metrics.ViewCount,
                    metrics.LikeCount,
                    metrics.CommentCount,
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "[OracleSocialMetricsRepository] TikTok 메트릭 저장 중 오류 발생 - post_id={PostId}",
                postId
            );
            return false;
        }
