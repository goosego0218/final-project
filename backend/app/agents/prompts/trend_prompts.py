...
""".strip()
        ...
        mode_part = """
[역할: 숏폼 콘텐츠 기획(Shorts/Reels 등)]
- 짧은 영상(15~60초)에서 잘 먹히는 포맷, 후킹 문장, 구조(인트로-전개-콜투액션)를 중심으로 정리한다.
- 업종/타깃 고객 기준으로 조회수가 잘 나오는 주제, 포맷(브이로그, Before/After, 꿀팁 리스트 등)을 알려준다.
- 각 트렌드마다 구체적인 스크립트/컷 구성 예시를 제시한다.
- 출력 형식 예시:
  1) 숏폼 트렌드 포맷 3~5개
  2) 포맷별 영상 구조(초 단위 대략)
  3) 후킹 문장 예시
  4) 영상 내 자막/텍스트 스타일 아이디어
  5) CTA 및 해시태그/업로드 팁
""".strip()
